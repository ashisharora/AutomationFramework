...
namespace OTPCMSTestAutomationFramework.CoreFramework.Helpers
{
    public class TestReportGenerator
    {
        ...
        private static string GetReportDirectory()
        {
            ...
            var buildDir = Environment.GetEnvironmentVariable("BUILD_ARTIFACTSTAGINGDIRECTORY");
            if (!string.IsNullOrEmpty(buildDir))
            {
                //Pipeline execution path
                var reportDir = Path.Combine(buildDir, "TestResults");
                Directory.CreateDirectory(reportDir);
                return reportDir;
            }

            //Local execution path
            var localDir = Path.Combine(Directory.GetCurrentDirectory(), "TestResults");
        ...
        public static void FinalizeReport()
        {
            try
            {
                ...
                if (_extent != null)
                {
                    ...
                    {
                        Console.WriteLine($"Report file created successfully at: {ReportPath}");
                        TestContext.Progress.WriteLine($"Report file created successfully at: {ReportPath}");
                    }
                    else
                    {
